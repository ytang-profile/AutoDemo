...
public class TC1KeywordSearchTest {
	...
	public void HomePageSearchKeyword() {
		...
		String keyword = ExcelUtils.getCellData(1, 3);
		...
		//Verify the results display
		WebDriverWait wait = new WebDriverWait(driver,5);
		wait.until(ExpectedConditions.visibilityOf(menu.resultList.get(0)));
		webObj.highlightElement(menu.resultList.get(0));
		
		Assert.assertTrue(menu.resultList.get(0).isDisplayed(),"No result is displayed");
		int resultCount = menu.resultList.size();
		System.out.println(resultCount);
		Assert.assertTrue(resultCount > 0,"No result is displayed");
